<?php 
	...
	if(isset($_GET['ct']) and isset($_GET['at'])) { 					// Check if temprature is present or not
		...
		$newData = "\n<record><ct>". $_GET['ct']. "</ct><at>". $_GET['at']. "</at><date>". date('d-m-Y H:i:s') . "</date></record>\n</records>"; 	
		$str = str_replace("</records>", $newData, $str);	// put the data in the end of the xml document
		
		file_put_contents('tempData-w7.xml', $str);			// write the file back to the server
